...
const Admin = () => {
  const investmentInfoRead = useSelector(
    (state) => state.auth.investmentInfoRead,
  )
  ...
  const logGet = async () => {
    ...
    console.log(
      'Developer fee in percentage==>',
      devFeePercentage?.toString(),
      'developer Fee in Wei =>',
      devFeeInWei.toString(),
      'PreslaeCount',
      presaleCount.toString(),
      'Presale adddress for id 0 ==>',
      preSaleAddress,
      'Admin is ==>',
      adminOwner,
    )
